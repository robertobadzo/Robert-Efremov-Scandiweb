  ...
  render() {
    ...
    return (
      <div>
        ...
        {cartProducts.slice(1).map((oneItem) => (<div key={uuid()} className='full-cart-card'>  <Query className="mc-card-father" query={GET_ONE_PRODUCT} variables={{ id: oneItem.id }}>
          {
            ({ data, loading, error }) => {
              ...
              return (
                <>
                  ...
                  <h1 className='mc-cart-card-price'>
                    {data.product.prices[this.props.hookCurrency].currency.symbol}{data.product.prices[this.props.hookCurrency].amount}
                  </h1>
                    ...
                    <div className='pdp-attributes flex-row'>
                      {data.product.attributes.map((item) =>
                        <div className='mc-item-name' key={uuid()}>{item.name}:<div className='mc-buttons-set-choose-attribute'>{item.items.map((el) =>
                          <div key={uuid()} className='mc-button-choose-attribute'
                           /*ON CLICK FUNCTION*/ onClick={() => this.props.dispatch({ type: "cc-aa", load: { id: data.product.id, value: { value: el.value, type: item.type } } })}
                  ...
                  <div className='mc-cart-card-image-numberof'>
                    ...
                    <img src={data.product.gallery[aImage]} className="mc-cart-image" ></img>
                    <div className='pdp-arrow' onClick={() => {aImage =+ 1; alert (aImage)}}></div>
                    <div className='pdp-arrow' onClick={() => {aImage =- 1; alert (aImage)}}></div>
                  </div>
                </>
              )
